...
if (no_woody=="0"){
  cat("* 巫云配置的密码为平台配置密码,需要与平台密码一致\n")
} else {
  cat("* 云平台配置的密码为平台配置密码,需要与平台密码一致\n")
}
```
```{r, results='asis',echo=FALSE}
cat(get_image("vpn-1.png"))
```
...
```{r, results='asis',echo=FALSE}
if (no_woody=="0"){
  cat("打开巫云管理平台<https://www.woody.vip/custom>并登陆\n")
} else {
  cat("打开云管理平台并登陆\n")
}
```
...
```{r, results='asis',echo=FALSE}
cat(get_image("vpn-2.png",public=TRUE))
```

设备列表页面可以看到自己公司所有设备的状态,列表所示的IP地址即为设备的VPN IP地址,可以用这个地址管理设备,也可以进行PLC上下载程序

```{r, results='asis',echo=FALSE}
cat(get_image("vpn-3.png",public=TRUE))
```
